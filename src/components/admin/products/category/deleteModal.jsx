...
export default function DeleteModal({ isOpen, onClose, category, product }) {
  ...
  return isOpen ? (
    <div className="fixed inset-0 p-4 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] overflow-auto font-[sans-serif]" onClick={onClose}>
      <div className="w-full cursor-default max-w-md bg-white shadow-lg rounded-md p-6 relative z-[1001]" onClick={handleOutsideClick}>
        {/* Close button (X) */}
        <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 cursor-pointer shrink-0 fill-black hover:fill-red-500 float-right" viewBox="0 0 320.591 320.591" onClick={onClose}>
        ...
        <div className="my-8 text-center">
          ...
          <div className='flex flex-col mt-4 text-xl text-slate-800 '>
            <span className="font-bold">{product?.ALBEDOtitulo}</span>
            <span className="font-bold">{category?.ALBEDOtitulo}</span>
        ...
        <div className="flex flex-col space-y-2">
          <button onClick={handleAddProduct} className="w-full px-6 py-2.5 rounded-md text-white text-sm font-semibold border-none outline-none bg-red-500 hover:bg-red-600 active:bg-red-500">
