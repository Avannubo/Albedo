...
export default function EditModal({ isOpen, onClose, category, product }) {
    ...
    const [productUrlCode, setProductUrlCode] = useState(product.url_Id);
    ...
    const handleAddProduct = async () => {
        ...
        // console.log(newProductUrlCode, category.products, productUrlCode);
        const urlIdExists = category.products.some(product => product.url_Id === newProductUrlCode); // Use 'url_Id' for comparison
        if (newProductUrlCode == productUrlCode) {
            setUrlCodeError(false);
        } else if (urlIdExists) {
        ...
        try {
            ...
            const uniqueImagePaths = Array.from(new Set([...productImages, ...savedImages]));
            ...
            // Delay to show loader for 3 seconds, then close modal
            // setTimeout(() => {
    ...
    return isOpen ? (
        <div className="fixed inset-0 p-4 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:max-h-screen before:bg-[rgba(0,0,0,0.5)] overflow-y-scroll font-[sans-serif]">
            <div className="w-full max-w-6xl bg-white p-5 shadow-lg rounded-md  relative">
                ...
                <svg onClick={handleOnClose} xmlns="http://www.w3.org/2000/svg" className="w-3.5 cursor-pointer shrink-0 fill-black hover:fill-red-500 float-right right-1" viewBox="0 0 320.591 320.591">
