...
export default function Filters({ list }) {
    ...
    const [searchData, setSearchData] = useState({ searchTerm: "", searchBy: "" });
    ...
    const searchFilterProducts = async (searchTerm, searchBy) => {
        // Implement your filtering logic here 
            await getFiltersListProducts(searchTerm);  
    };
    // useEffect(() => {
    //     // Function to be executed at an interval
    //     const intervalId = setInterval(() => {
    //         searchFilterProducts(categoryFilter, searchData.searchBy);
    //     }, 35000); // 15-second interval

    //     // Cleanup the interval on component unmount
    //     return () => clearInterval(intervalId);
    // }, [categoryFilter, searchData.searchBy]); // Dependency array to re-apply interval if these change

    return (
        <div className="flex flex-row justify-end space-x-2 w-auto">
            <select
                value={categoryFilter}
                onChange={(e) => onCategoryChange(e.target.value)}
                className="w-auto h-[42px] px-1.5 py-1 border-2 border-[#304590] rounded-lg focus:outline-none focus:border-[#304590]"
            >
            ...
            {/* Button to reload filter */}
            <button
                onClick={() => searchFilterProducts(categoryFilter, searchData.searchBy)}
                className="whitespace-nowrap   p-2 bg-[#304587] hover:bg-[#475caa] text-white rounded-md"
            >
                <svg
                    width="24px"
                    height="24px"
