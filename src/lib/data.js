...
export async function requireContent() {
    const res = await fs.readFile('public/data/data.json', 'utf8')
        ...
        const deleteRecursive = async (categoryList) => {
            for (let i = 0; i < categoryList.length; i++) {
                ...
                if (category.id === categoryToDeleteId) {
                    ...
                    await fs.writeFile('public/data/data.json', JSON.stringify({ categories, deletedContent }));
...
export async function addCategory(name, description) {
    ...
    try {
        const data = await readFile('public/data/data.json', 'utf8');
        ...
        await writeFile('public/data/data.json', JSON.stringify(jsonData));
        revalidatePath('/admin/categories');
